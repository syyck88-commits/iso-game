...
export class RenderManager {
  ...
  // Base offsets (Centering the grid initially)
  ...
  // Get effective offset for World Space calculations
  get offsetX() { return this.baseOffsetX; }
  get offsetY() { return this.baseOffsetY; }
  ...
  resize() {
    ...
    this.baseOffsetX = this.width / 2;
    this.baseOffsetY = (this.height / 2) - (gridPixelHeight / 2) + 50; 
  ...
  // Helper to convert raw Screen Mouse X/Y to World X/Y (taking zoom/pan into account)
  ...
  update(dt: number) {
      ...
      if (Math.abs(zoomDiff) > 0.001) {
          this.zoom += zoomDiff * 0.1; // Smooth factor
      } else {
  ...
  draw() {
    ...
    ctx.translate(-cx, -cy);
    // 4. Pan
    ctx.translate(this.pan.x + sx, this.pan.y + sy);
  ...
  postDraw() {
     ...
     if (hover && selectedType) {
        ...
        let fillColor = 'rgba(74, 222, 128, 0.4)';
    ...
    // Close the camera transform saved in draw()
    ctx.restore();
  }
  ...
  private drawBaseTile(ctx: CanvasRenderingContext2D, x: number, y: number, pos: Vector2, tileType: number) {
    ...
    if (depth > 0) {
        ...
        ctx.fillStyle = this.darkenColor(color, 0.85); 
        ctx.beginPath();
  ...
  private drawRoads(ctx: CanvasRenderingContext2D) {
      const roadColor = '#44403c'; 
      const roadBorder = '#292524'; 
      const roadWidth = 14; 
  ...
  private drawRoadConnections(ctx: CanvasRenderingContext2D, color: string, width: number) {
      ...
      for (let y = 0; y < GRID_SIZE; y++) {
          for (let x = 0; x < GRID_SIZE; x++) {
              ...
              ctx.moveTo(currentPos.x, currentPos.y);
              ctx.lineTo(currentPos.x + 0.1, currentPos.y); 
          }
      }
      ctx.stroke();
  }
